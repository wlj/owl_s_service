...
public class RepeatWhileGraphModelImpl extends ProcessGraphModelImpl {
	...
	Logger logger;
	public RepeatWhileGraphModelImpl() {
		// TODO Auto-generated constructor stub
		nodeList = new ArrayList<Node>();
		edgeList = new ArrayList<Edge>();
		
		logger = Logger.getLogger(cn.edu.pku.ss.matchmaker.process.impl.RepeatWhileGraphModelImpl.class);
	}
	...
	public boolean createModel(CompositeProcess process) {
		...
		if (StructureType.getStructureType(construct).equals(StructureType.REPEATWHILE) == false) {
			logger.error("process: " + process.getURI());
			logger.error("structure type is not Repeat-While");
			return false;
		}

	    ControlConstruct whileConstruct = ((RepeatWhile)construct).getWhileProcess();
	    beginNode = new StructureNodeImpl(NodeType.STRUCTURE, StructureType.REPEATWHILE, BeginEndType.BEGIN);
	    endNode = new StructureNodeImpl(NodeType.STRUCTURE, StructureType.REPEATWHILE, BeginEndType.END);

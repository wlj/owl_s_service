...
public class SplitJoinGraphModelImpl extends ProcessGraphModelImpl {
	...
	Logger logger;
	public SplitJoinGraphModelImpl() {
		// TODO Auto-generated constructor stub
		nodeList = new ArrayList<Node>();
		edgeList = new ArrayList<Edge>();
		
		logger = Logger.getLogger(cn.edu.pku.ss.matchmaker.process.impl.SplitJoinGraphModelImpl.class);
	}
	...
	public boolean createModel(CompositeProcess process) {
		...
		if (StructureType.getStructureType(construct).equals(StructureType.SPLITJOIN) == false) {
			logger.error("process: " + process.getURI());
			logger.error("structure type is not Split-Join");
			return false;
		}
		...
		beginNode = new StructureNodeImpl(NodeType.STRUCTURE, StructureType.SPLITJOIN, BeginEndType.BEGIN);
		endNode = new StructureNodeImpl(NodeType.STRUCTURE, StructureType.SPLITJOIN, BeginEndType.END);
		...
		while (components != null) {
			...
			if (tmpProcess != null && tmpProcess.isAtomic()) {
				...
				// add edges
				Edge beginEdge = new EdgeImpl(beginNode, currentServiceNode, 0);
				Edge endEdge = new EdgeImpl(currentServiceNode, endNode, 0);

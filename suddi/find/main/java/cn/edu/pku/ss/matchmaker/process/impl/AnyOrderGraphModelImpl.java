...
public class AnyOrderGraphModelImpl extends ProcessGraphModelImpl {
	private List<Node> nodeList;
	private List<Edge> edgeList;
	private Node beginNode;
	private Node endNode;
	Logger logger;
	public AnyOrderGraphModelImpl() {
		// TODO Auto-generated constructor stub
		nodeList = new ArrayList<Node>();
		edgeList = new ArrayList<Edge>();
		
		logger = Logger.getLogger(cn.edu.pku.ss.matchmaker.process.impl.AnyOrderGraphModelImpl.class);
	}
	...
	public boolean createModel(CompositeProcess process) {
		...
		if (StructureType.getStructureType(construct).equals(StructureType.ANYORDER) == false) {
			logger.error("process: " + process.getURI());
			logger.error("structure type is not AnyOrder");
			return false;
		}
		...
		beginNode = new StructureNodeImpl(NodeType.STRUCTURE, StructureType.ANYORDER, BeginEndType.BEGIN);
		...
		endNode = new StructureNodeImpl(NodeType.STRUCTURE, StructureType.ANYORDER, BeginEndType.END);
		endNode.getPrecursor().add(preNode);
		preNode.getSuccessor().add(endNode);
		Edge edge = new EdgeImpl(preNode, endNode, 0);

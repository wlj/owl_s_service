...
public class ProcessInquiryImpl implements ProcessInquiry {

	/**
	 * 
	 */
	private static final long serialVersionUID = -890074459505176538L;

	public ProcessInquiryImpl()
	{}
	
	public Process ProcessInquiryEntry(int process_id, String flag) {
		// TODO Auto-generated method stub
		
		String sql = ProcessSql.sql_process_inquiry_by_process_id;
		Object[] params = new Object[]{ process_id };
		
		ResultSet process_set = SQLHelper.ExecuteQueryRtnSet(sql, params);
		
		return ProcessInquiryEntryOpe(process_set);
	}
	...
	private Process ProcessInquiryEntryOpe(ResultSet process_set) {
		// TODO Auto-generated method stub
		...
		try {
			if(process_set.next())
			{
				int process_id = Integer.parseInt(process_set.getString("id"));
				...
				int cc_type = Integer.parseInt(process_set.getString("cc_type"));
				...
				case 2:
					...
					ProcessRestoreOpe(process, process_id); //iterate
					break;
				}
			}
	...
	public void ProcessRestoreOpe(Process root_process, int process_id)
	{
		ControlConstructImpl cci = ProcessResource.GetCCIFromProcess(root_process);
	...
	private void RestoreForSequenceImpl(ControlConstructImpl cci, int parent_id) throws SQLException {
		// TODO Auto-generated method stub
		...
		while(relation_set.next())
		{
			...
			int process_id = Integer.parseInt(relation_set.getString("process_id"));
			...
			{
				System.out.println("error: there are more than one line of process item");
				return;
			}
	...
	private void RestoreForIfThenElseImpl(ControlConstructImpl cci, int parent_id) throws NumberFormatException, SQLException {
		// TODO Auto-generated method stub
		...
		{
			int process_id = Integer.parseInt(relation_set.getString("process_id"));
			int relation_type = Integer.parseInt(relation_set.getString("type"));
			...
			if(process_set.next())
			{
				...
				case 1:
					...
					else if(relation_type == 2)
					{
						...
						((PerformImpl)thenControlConstruct).setProcess(process);
						((IfThenElseImpl)cci).setThen(thenControlConstruct);
					}
					else
					{
						elseControlConstruct = new PerformImpl();
						((PerformImpl)elseControlConstruct).setProcess(process);
	...
	private void RestroForIfCondition(ControlConstructImpl cci, int parent_id) {
		...
		try {
			...
			if(condition_set.next())
			{
				...
				LogicLanguage lang = new LogicLanguageImpl(condition_set.getString("expression_language"));
				ifcondition.setExpressionLanguage(lang);
				ifcondition.setExpressionBody(condition_set.getString("expression_body"));
			}
		...
		try {
			...
			while(result_set.next())
			{
				...
				while(condition_set.next())
				{
					...
					int type = Integer.parseInt(condition_set.getString("type"));
					if(type == 4)
					{
						incondition = new ConditionImpl(url);
						incondition.setExpressionBody(condition_set.getString("expression_body"));
					...
					else if(type == 3)
					{
						...
						effect.setExpressionBody(condition_set.getString("expression_body"));
	...
	private PreConditionList GetPreConditionListByProcessID(int process_id) {
		...
		try {
			...
			{
				precondition = new ConditionImpl(precondition_set.getString("uri"));
				
				LogicLanguage lang = new LogicLanguageImpl(precondition_set.getString("expression_language"));
				precondition.setExpressionLanguage(lang);
				precondition.setExpressionBody(precondition_set.getString("expression_body"));
	...
	private void RestoreForSplitJoinImpl(ControlConstructImpl cci, int process_id) {
		// TODO Auto-generated method stub
	...
	private void RestoreForRepeatUntilImpl(ControlConstructImpl cci, int process_id) {
		// TODO Auto-generated method stub
		
	}

	private void RestoreForRepeatWhileImpl(ControlConstructImpl cci, int parent_id) throws SQLException {
		...
		while(relation_set.next())
		{
			...
			if(process_set.next())
			{
				...
				case 1:
					...
					((RepeatWhileImpl)cci).setWhileProcess(whileControlConstruct);
					break;
				case 2:
					process = new CompositeProcessImpl(uri);
	...
	private void RestoreForWhileCondition(ControlConstructImpl cci, int process_id) {
		...
		try {
			...
			if(condition_set.next())
			{
				whileCondition = new ConditionImpl(condition_set.getString("uri"));
				LogicLanguage lang = new LogicLanguageImpl(condition_set.getString("expression_language"));
				whileCondition.setExpressionLanguage(lang);
				whileCondition.setExpressionBody(condition_set.getString("expression_body"));
			}
	...
	public Parameter ParameterInquiry(int param_id) {
		...
		try {
			if(rs_param.next())
			{
				...
				int is_output = Integer.parseInt(rs_param.getString("is_output"));
